...
function onIntent(intentRequest, session, callback) {
  ...
  if ("GetAddressIntent" === intentName) {
    ...
    if (session.attributes.previousPlace === "Get Address") {
      getLatLng(intent, session, callback);
    } else if (session.attributes.previousPlace === "Closest Offices") {
      closestOffices(intent, session, callback);
    }
...
function getAddress(intent, session, callback) {
  ...
  if (streetNumber && streetAddress && city && state && zip) {
    googleAddress = streetNumber + " " + capitalizeFirst(streetAddress) + " " + capitalizeFirst(city) + ", " + state + " " + zip;
    userAddress = streetNumber + " " + capitalizeFirst(streetAddress) + ", " + capitalizeFirst(city) + ", " + state + " " + zip;
    speechOutput = "You would like to know the closest DMV offices to " + userAddress + "? Say, yes to convert the address to coordinates, or say, no to exit. ";
...
function getLatLng(intent, session, callback) {
  var sessionAttributes = {};
  var CARD_TITLE = "Converting Your Address To Coordinates";
  var userAddress = session.attributes.userAddress;
  var googleAddress = session.attributes.googleAddress;
  var shouldEndSession = false;
  var speechOutput;
  var repromptText;
  var userLat;
  ...
  speechOutput = userAddress + " converts to a latitude of " + userLat + " and a longitude of " + userLat + ". Say, calculate to find out what the closest DMV offices to you are. ";
  repromptText = "Say, calculate to find out what the closest DMV offices to you are. ";

  sessionAttributes = {
    "speechOutput": speechOutput,
    "repromptText": repromptText,
    "userLat": userLat,
    "previousPlace": "Get Lat Lng"
  };
...
function closestOffices(intent, session, callback) {
  ...
  for (var i = 0; i < dmvOffice.length; i++) {
    ...
    homeLat = parseInt(session.attributes.userLat);
    homeLng = parseInt(session.attributes.userLng);
